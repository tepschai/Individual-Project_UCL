...
def lstm(scaler, x, y, x_train, x_test, y_train, y_test, obj, optimizer, lag, net):
    ...
    # record results
    record = pd.DataFrame({'Obj':[obj], 'Optimizer':[optimizer], 'Lag size':[lag], 'Neurons':[net], 'MSE':[mse], 
              'RMSE':[rmse], 'MSE_pred':[mse_pred], 'RMSE_pred':[rmse_pred]})
    
    #save raw data
    test = pd.DataFrame(np.zeros([y_test.shape[0], y_test.shape[1]*2]), 
                        columns=pd.MultiIndex.from_product([['test','pred'],[1,2,3,4,5,6]]))
    
    pred = pd.DataFrame(np.zeros([y_true.shape[0], y_true.shape[1]*2]), 
                        columns=pd.MultiIndex.from_product([['true','pred'],[1,2,3,4,5,6]]))
...
if __name__ == '__main__':
    ...
    records = pd.DataFrame(columns=['Obj', 'Optimizer', 'Lag size', 'Neurons', 'MSE', 'RMSE', 'MSE_pred', 'RMSE_pred'])
    ...
    records.to_csv(f'forward_{step}_log.csv')
